...
export default class Feeds extends Command {
      ...
      return new SlashCommandBuilder()
         ...
         .addSubcommand((command) =>
            command
               ...
               .addChannelOption((option) =>
                  option.setName("channel").setDescription("Channel to add").setRequired(true)
               )
               .addStringOption((option) =>
                  option.setName("message").setDescription("A message that will be sent by bot when there is new tweet")
               )
               .addBooleanOption((option) =>
                  option.setName("replies").setDescription("Show replies when you get notification")
               )
               .addBooleanOption((option) =>
                  option.setName("retweets").setDescription("Show retweets when you get notification")
               )
         )
         .addSubcommand((command) =>
            command
               ...
               .addChannelOption((option) =>
                  option.setName("channel").setDescription("Channel to remove").setRequired(true)
               )
         )
         ...
         .addSubcommand((command) =>
            command
               ...
               .addStringOption((option) =>
                  option.setName("username").setDescription("Feed's username to view details").setRequired(true)
               )
               .addChannelOption((option) =>
                  option.setName("channel").setDescription("Feed's Channel to view details").setRequired(true)
               )
         )
         .addSubcommand((command) =>
            command
               ...
               .addStringOption((option) =>
                  option.setName("feed_id").setDescription("Feeds id to edit").setRequired(true)
               )
   ...
   public async run(interaction: CommandInteraction): Promise<any> {
      ...
      if ((interaction.member?.permissions as Permissions).has(Permissions.FLAGS.MANAGE_GUILD)) {
         ...
         let showReplies = interaction.options.getBoolean("replies")
            ? interaction.options.getBoolean("replies")
            : false;
         let showRetweets = interaction.options.getBoolean("retweets")
            ? interaction.options.getBoolean("retweets")
            : false;
         ...
         if (subcommand === "add") {
            ...
            try {
               ...
               if (guild.feeds.find((feed: any) => feed.channel === channel.id && feed.twitterUserId === user.id)) {
                  return iFollowUp(interaction, {
                     content:
                        emojis.f +
                        `${username}’s feed you are trying to add at <#${channel.id}> is currently at feed list`,
                     ephemeral: true,
                  });
               }
               !(await getWebhookData(interaction.client, channel.id)) &&
                  (await createWebhook(interaction.client, channel as TextChannel, guildId as string));
               ...
               //@ts-ignore
               interaction.client.streamClient.restart(true);
            } catch (e: any) {
               if (e.code === 50013) {
                  iFollowUp(interaction, {
                     content:
                        emojis.f +
                        "Tweetcord doen't have permissions to create webhooks. Grant permissions to continue.",
                  });
               } else if (e.code === 30007) {
                  iFollowUp(interaction, {
                     content:
                        emojis.f +
                        "The channel you are trying to add a feed to has the maximum number of webhooks (10). If you want to add a feed, you need to delete one of the webhooks.",
                  });
               } else {
                  console.log(e);
            ...
            try {
               ...
               let find = guild.feeds.find(
                  (user: any) => data.id === user.twitterUserId && user.channel === channel.id
               );
               if (!find) {
                  return iFollowUp(interaction, {
               ...
               if (guild.feeds.filter((feed: any) => feed.channel === channel.id).length - 1 === 0)
                  deleteWebhook(interaction.client, webhook.id, webhook.webhookId, webhook.webhookToken);
            ...
            for (let feed of feeds) {
               ...
               if (find) {
                  ...
                  feedsUnique.push({
                     name: feed.channel,
                     value: [data.find((user: any) => user.id === feed.twitterUserId)?.username],
                     inline: true,
                  });
               }
            }
            ...
            feedsUnique.map((feed: any) => {
               (feed.name = interaction.guild?.channels.cache.get(feed.name)?.name),
                  (feed.value = feed.value.map((str: string) => "`" + str + "`").join("\n"));
            });
            ...
            if (
               !message &&
               interaction.options.getBoolean("replies") === null &&
               interaction.options.getBoolean("retweets") === null
            )
               return iFollowUp(interaction, { content: emojis.f + "You have to provide at least 1 option." });
            let { feeds } = guild;
            let find = feeds.find((feed: any) => feed.id === feedId);
            if (!find)
               return iFollowUp(interaction, {
                  content: emojis.f + "Can't find any feeds with this id (`" + feedId + "`)",
               });
            if (!message) message = find.message;
            if (message === "tweetcord_remove_message") message = "";
            if (message === find.message && showReplies === find.replies && showRetweets === find.retweets)
               return iFollowUp(interaction, {
                  content: emojis.f + "Provided options same with current feed's options",
               });
