  ...
  public async run(interaction: CommandInteraction): Promise<any> {
    ...
    if ((interaction.member?.permissions as Permissions).has(Permissions.FLAGS.MANAGE_GUILD)) {
      ...
      if (subcommand === "add") {
        ...
        if (!message && interaction.options.getBoolean("replies") === null && interaction.options.getBoolean("retweets") === null)
        ...
        if (message === find.message && showReplies === find.replies && showRetweets === find.retweets)
          return iFollowUp(interaction, { content: emojis.f + "Provided options same with current feed's options" });
