    ...
    public async run(interaction: CommandInteraction): Promise<void> {
        ...
        if (commandName && cmds.find((cmd) => cmd.name === commandName)) {
            ...
            embed = {
                ...
                description: `If you need help about command, just use \`help [command]\` or join ${hyperlink("support server.", "https://discord.com/invite/tV22Kvj")}. \n\n **Commands:** \n ${blockQuote(
                    cmds.map((command) => `\`${command.name}\``).join(", ")
                )} \n `,
                footer: {
                    iconURL: interaction.user.displayAvatarURL(),
                    text: "Do not put [] around command arguments.",
