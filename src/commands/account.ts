...
export default class Account extends Command {
         ...
         .addSubcommand((command) =>
            command
               ...
               .addStringOption((option) =>
                  option.setName("text").setDescription("The text of the tweet.").setRequired(true)
               )
         )
         .addSubcommand((command) =>
            command
               ...
               .addStringOption((option) =>
                  option.setName("username").setDescription("User's username to follow.").setRequired(true)
               )
         )
         .addSubcommand((command) =>
            command
               ...
               .addStringOption((option) =>
                  option.setName("username").setDescription("User's username to unfollow.").setRequired(true)
               )
         )
         .addSubcommand((command) =>
            command
               ...
               .addStringOption((option) =>
                  option.setName("username").setDescription("User's username to block.").setRequired(true)
               )
         )
         .addSubcommand((command) =>
            command
               ...
               .addStringOption((option) =>
                  option.setName("username").setDescription("User's username to unblock.").setRequired(true)
               )
         )
         .addSubcommand((command) =>
            command.setName("ratelimits").setDescription("Get information about your current rate limits.")
         );
   }
   public async run(interaction: CommandInteraction): Promise<void> {
      ...
      if (checkUserRateLimits(interaction, userDB))
         return iFollowUp(interaction, { content: emojis.f + "You are being rate limited. Please try again later." });
      if (subcommand === "info") {
         const embed: MessageEmbedOptions = {
            title: `${Util.escapeMarkdown(cureentTwitterUser.name)} ${
               cureentTwitterUser.verified ? Formatters.formatEmoji("743873088185172108") : ""
            }`,
            author: {
         ...
         if (!user || !user.data) return iFollowUp(interaction, { content: "User not found." });
         twitterClient.v2
         ...
         twitterClient.v2
            ...
            .then((r) => {
               if (!(r as UserV2UnfollowResult).data.following)
                  return iFollowUp(interaction, {
                     content: emojis.t + "You already not following " + user.data.name + ".",
                  });
         ...
         twitterClient.v2
            ...
            .then((r) => {
               if (!(r as UserV2BlockResult).data.blocking)
                  return iFollowUp(interaction, {
                     content: emojis.t + "You already not blocked " + user.data.name + ".",
                  });
