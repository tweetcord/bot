...
export default class App {
      ...
      this.app
         ...
         .post("/api/guild", async (req: Request, res: Response) => {
            ...
            if (!(guild.permissions & 8))
               return res.send({ error: 4031, message: "User doesn't have permissions to manage this guild." });
         ...
         .post("/api/feeds", async (req: Request, res: Response) => {
            let feed = req.body;
            if (!feed || !feed.channel || !feed.guildId || !feed.twitterUserId)
               return res.send({ error: 4045, message: "Missing required fields." });
            let userData = await this.getUserData(req, res);
            if (userData.error) return res.send(userData);
            let { guilds } = userData;
            if (!(guilds.find((guild: any) => guild.id === feed.guildId).permissions & 8))
               return res.send({ error: 4031, message: "User doesn't have permissions to manage this guild." });
            ...
            if (feed.twitterUserId === "newUser") {
               let twitterUser = await this.client.twitter.v2
                  .userByUsername(feed.user.username.replace("@", ""))
                  .catch(() => {});
         ...
         .get("/auth/twitter", async (req: Request, res: Response) => {
            ...
            const authLink = await this.client.twitter.generateAuthLink(
               "https://api.tweetcord.xyz/auth/twitter/callback",
               { linkMode: "authorize" }
            );
            let options = {
               maxAge: 1000 * 60 * 30,
         ...
         .get("/auth/twitter/callback", async (req: Request, res: Response) => {
            ...
            userClient
               ...
               .then(async ({ client: _loggedClient, accessToken, accessSecret }) => {
         ...
         .post("/getTwitterUserData", async (req: Request, res: Response) => {
            ...
            if (!cureentTwitterUser)
               return res.send({ error: 4044, message: "Can't find authorized Twitter account." });
   ...
   private convertIDtoTwitterUser = async (guildDb: any) => {
      ...
      if (idArr.length > 0) {
         let { data: users } = await this.client.twitter.v2.users(idArr, {
